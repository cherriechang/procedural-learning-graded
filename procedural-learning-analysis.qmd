...
```{r}
# last block data model, for each of the matrix_size
data.m_h1h2 <- df %>%
  filter(block==EXPERIMENT_CONFIG[["n_blocks"]]-1, !is.infinite(log_rt))



data_list <- split(data.m_h1h2, data.m_h1h2$matrix_size)

#names(data_list) <- unique(data.m_h1h2$matrix_size)
...
### single condition version (deprecated)
