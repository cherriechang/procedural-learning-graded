...
## Modeling Approach
We use linear mixed effects regression (LMER) to model log-transformed RTs and generalized linear mixed effects regression (GLMER) with a binomial link function to model accuracy. Models are fit using the `lmerTest` package in R.P-values were obtained using the Satterthwaite approximation for degrees of freedom, as implemented in the `lmerTest` package (@kuznetsova_lmertest_2017).
...
We first determined the optimal effects structure using final-block data, comparing candidate models by summing AIC values across all five matrix sizes. This comparison favored the model: `log_rt ~ surprisal_c * prev_entropy_c + is_repetition + (surprisal_c * prev_entropy_c || subject_id) + (1 | position)`

We then fit this model structure each block in each matrix size condition separately, further pruning the random effets structure until all models converge. This pruning favored a simpler random effects structure, resulting in the following final model: `log_rt ~ surprisal_c * prev_entropy_c + is_repetition + (prev_entropy_c| subject_id) + (1 | position)`

In the final block, all predicted effects were significant across all five matrix size conditions (Table \ref{tab:h1h2-results}). Higher surprisal transitions elicited slower responses, as did transitions following higher-entropy positions. The negative interaction indicates that surprisal effects were stronger following low-entropy (more predictable) positions, consistent with prediction-based learning. Repetitions showed faster responses, likely reflecting motor facilitation. This structure converged without singularity across all 100 models (20 blocks × 5 conditions) and is therefore used for all subsequent analyses.
...
cat("\\end{tabular*}\n\n")
cat("\\footnotesize\n")
cat("\\textit{Note.} Mean $\\beta$ averaged across 5 matrix size conditions (4×4, 5×5, 6×6, 7×7, 8×8). \\\\\n")
cat("Sig. = number of conditions with $p$ < .05; *** = all conditions $p$ < .001.\n")
cat("\\end{table}\n")
```
