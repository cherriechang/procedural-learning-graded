...
![The interaction between surprisal and previous entropy effect across learning blocks]("../plots/h3_surprisal_by_block_2.png"){#fig-h3-suprprisal-by-block width=100%}

![Previous entropy effect across learning blocks]("../plots/h3_entropy_by_block_2.png"){#fig-h3-entropy-by-block width=100%}

![Surprisal effect across learning blocks]("../plots/h3_interaction_by_block_2.png"){#fig-h3-interaction-by-block width=100%}
...
Our results provide clear evidence that learners are sensitive to the graded statistical structure embedded in serial reaction time tasks across multiple state space sizes. We found robust effects of surprisal and entropy on reaction times at every matrix size tested (4×4, 5×5, 6×6, 7×7, and 8×8), demonstrating that participants learned fine-grained transition probabilities rather than simply practicing motor responses or learning that "some transitions are hard." Critically, we observed no interaction between surprisal and entropy effects, indicating that the effect of surprisal persists even when entropy is relatively high. This independence is particularly impressive because it shows that learners are not merely learning global difficulty levels, but are actually tracking which specific transitions are more or less likely even in high-uncertainty contexts.

## Scaling Properties and Training Duration

Importantly, the effects of surprisal and entropy did not rapidly approach zero as matrix size increased. This suggests that sensitivity to statistical structure scales beyond the typical 4-position SRTT paradigm, at least within the range we tested. While there remains considerable distance between an 8×8 transition matrix and the scale of statistical structure found in natural language or other real-world domains, our findings are encouraging given the brevity of training: participants completed only 20 blocks over approximately 20-40 minutes, receiving just 10 exposures per position per block. Despite this limited exposure, learners demonstrated robust sensitivity to graded statistical patterns across increasing state space complexity.

## Limitations

Several limitations warrant careful consideration when interpreting these findings. First, our transition matrices remain relatively small compared to the state spaces encountered in natural domains like language. Second, the short duration of our task means it does not fully represent the extended timescales typical of real-world procedural learning. Third, we made methodological decisions—particularly regarding how to handle random effects structures when fitting separate models to individual blocks—that may affect the quantitative estimates of effect sizes, though we believe these are unlikely to alter the qualitative pattern of results. These modeling choices were necessary to maintain convergence and interpretability across our per-block analyses, but future work should explore the robustness of specific effect size estimates to different modeling approaches.

## Implications for Understanding Procedural Learning

Taking these results at face value, they have important implications for understanding the relationship between procedural learning and language acquisition. Our task represents a substantially more complex design than traditionally used in procedural learning research, yet we still observe fine-grained sensitivity to surprisal and entropy that converges with recent quantitative analyses of language processing [@linzen_how_2019; @smith_role_2013]. This convergence increases our confidence that serial reaction time tasks can serve as valid models for language learning processes. More broadly, it increases the plausibility that procedural learning mechanisms and language learning rely on shared underlying cognitive systems, consistent with theoretical proposals linking the two [@ullman_neurocognition_2016; @ullman_declarative_2004].

The preservation of both surprisal and entropy effects across varying levels of structural complexity is particularly noteworthy. It suggests that the statistical learning mechanisms engaged in these tasks are not simply detecting binary patterns or memorizing sequences, but rather building probabilistic models that capture graded distributional information. This capacity for nuanced statistical representation in a procedural learning context provides a bridge between the traditionally separate literatures on implicit sequence learning and statistical language learning.

## Future Directions

Future research should pursue at least two important directions. First, additional scaling is needed to further test the boundaries of procedural statistical learning. One promising approach is to embed these learning paradigms within engaging game environments that can sustain participant motivation over extended training periods with larger state spaces [@sumers_revisiting_2024]. Such designs would allow us to test whether the scaling properties we observe continue to hold as we approach state space sizes more representative of natural language.

Second, examining procedural learning across the lifespan would provide a crucial test of the hypothesized link between procedural learning and language acquisition. If these processes share underlying mechanisms, we should observe parallel patterns of age-related change. Specifically, comparing younger and older adults' sensitivity to statistical structure in procedural learning tasks—and relating these patterns to what is known about age-related changes in language learning [@ullman_neurocognition_2016; @hartshorne_critical_2018]—would offer compelling evidence for or against shared cognitive systems. Such lifespan comparisons could also inform practical questions about the optimal timing and methods for language instruction across development.
