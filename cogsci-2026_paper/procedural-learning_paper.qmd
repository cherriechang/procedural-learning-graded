...
\end{figure}
```
Our experiment deviates from other SRTT designs in how the position sequence and its probabilistic structure is constructed. In each experiment run, we use a first-order Markov process to generate the sequence of positions the mole appears in. This process is defined by a transition matrix, where each entry in the matrix defines the probability of transitioning from one position to another. Following the five conditions in our experiment, we designed five "original" transition matrices of sizes 4x4, 5x5, 6x6, 7x7, and 8x8 to describe the transition probabilities for each number of positions. These matrices were constructed to have graded and increasing levels of entropy across rows, such that position 1 has lowest entropy---having the most predictable next-position-transitions, followed by the second, with the last position, position $n$, having highest entropy. For example, in the 4×4 matrix (Figure \ref{fig:4x4-state-transition-graph}), position 1 has a high probability of transitioning to position 2 ($p_{1 \rightarrow 2}=$ `r sprintf("%.2f", floor(transition_matrix_4x4[1,2] * 100) / 100)`), a low probability of transitioning to position 1 ($p_{1 \rightarrow 1}=$ `r sprintf("%.2f", floor(transition_matrix_4x4[1,1] * 100) / 100)`), and never transitions to positions 3 or 4; while position 4 has a more uniform distribution of transition probabilities to all other positions ($p_{4 \rightarrow 1}=$ `r sprintf("%.2f", floor(transition_matrix_4x4[4,1] * 100) / 100)`, $p_{4 \rightarrow 2}=$ `r sprintf("%.2f", floor(transition_matrix_4x4[4,2] * 100) / 100)`, $p_{4 \rightarrow 3}=$ `r sprintf("%.2f", floor(transition_matrix_4x4[4,3] * 100) / 100)`, $p_{4 \rightarrow 4}=$ `r sprintf("%.2f", floor(transition_matrix_4x4[4,4] * 100) / 100)`). This gives rise to a gradient of entropy values across the positions, resulting in a more continuously distributed statistical structure underlying the sequence of positions the stimulus mole appears in, allowing us to investigate how the procedural learning system differentially picks up varying levels of probability in a given statistical structure.
...
cat("Prop. Sig. = conditions with $p$ < .05;\n*** = all conditions $p$ < .001.\n")
cat("\\end{table}\n")
```
...
![Surprisal effect across learning blocks per condition.]("../plots/h3_surprisal_by_block.png"){#fig-h3-surprisal-by-block width=100% fig-pos="!t"}
...
![Previous position entropy effect across learning blocks per condition.]("../plots/h3_entropy_by_block.png"){#fig-h3-entropy-by-block width=100% fig-pos="!t"}
...
![Surprisal x previous position entropy effect across learning blocks per condition.]("../plots/h3_interaction_by_block.png"){#fig-h3-interaction-by-block width=100% fig-pos="!t"}
