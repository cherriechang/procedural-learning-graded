...
files_5x5 <- osf_retrieve_node(EXPERIMENT_CONFIG$osf_id_5x5) %>%
  osf_ls_files(n_max = Inf) %>%
  osf_download(path = "data/5x5-data", conflicts="skip")

files_6x6 <- osf_retrieve_node(EXPERIMENT_CONFIG$osf_id_6x6) %>%
  osf_ls_files(n_max = Inf) %>%
  osf_download(path = "data/6x6-data", conflicts="skip")

files_7x7 <- osf_retrieve_node(EXPERIMENT_CONFIG$osf_id_7x7) %>%
  osf_ls_files(n_max = Inf) %>%
  osf_download(path = "data/7x7-data", conflicts="skip")

files_8x8 <- osf_retrieve_node(EXPERIMENT_CONFIG$osf_id_8x8) %>%
  osf_ls_files(n_max = Inf) %>%
  osf_download(path = "data/8x8-data", conflicts="skip")
```
...
## Modeling Approach
We use linear mixed effects regression (LMER) to model log-transformed RTs and generalized linear mixed effects regression (GLMER) with a binomial link function to model accuracy. We start with models including random intercepts for subject, position and whether the trial is a repetition, as well as random slopes for key predictors by subject where appropriate. Models are fit using the `lmerTest` package in R. [TODO: consider adding optimizer] We first test for the learning effect across blocks (H0), then examine the effects of surprisal and previous positional entropy (H1 & H2) in the final block, pruning effects where applicable to narrow in on the best-fitting effects structure before analyzing learning trajectories across blocks, and finally test for moderation by number of positions (H3).
...
base model: `log_rt ~ surprisal_c * prev_entropy_c + is_repetition + (surprisal_c * prev_entropy_c + is_repetition | subject_id) + (1 | position)`
...
AIC(m_surprisal_x_entropy_rt, m_surprisal_x_entropy_rt_rand_slope_1, m_surprisal_x_entropy_rt_rand_slope_2, m_surprisal_x_entropy_rt_rand_slope_3)
...
AIC(m_surprisal_x_entropy_rt_rand_slope_1, m_surprisal_x_entropy_rt_fixed_effects_1, m_surprisal_x_entropy_rt_fixed_effects_2)
```
