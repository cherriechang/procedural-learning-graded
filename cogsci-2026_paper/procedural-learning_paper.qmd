...
# Summary across all 100 models (all blocks, all matrix sizes)
h1h2_summary <- h1h2_all_stats %>%
    group_by(term) %>%
    summarise(
...
In all blocks across all matrix sizes, this model revealed significant effects for all predictors (Table \ref{tab:h1h2-results}). This structure converged without singularity across all 100 models (20 blocks × 5 conditions) and is therefore used for all subsequent analyses.
...
cat("\\begin{table}[t]\n")
cat("\\centering\n")
cat("\\caption{Fixed effects from all per-block models (100 total: 20 blocks × 5 matrix sizes)}\n")
cat("\\label{tab:h1h2-results}\n\n")
cat("\\begin{tabular*}{\\columnwidth}{@{} l @{\\extracolsep{\\fill}} r r r @{}}\n")
cat("\\toprule\n")
cat("Predictor & Mean $\\beta$ & Mean \\textit{t} & \\textit{p} \\\\\n")
cat("\\midrule\n")

h1h2_latex_rows <- h1h2_summary %>%
    mutate(
    ...
    transmute(
        row = paste0(term_label, " & ", sprintf("%.3f", mean_estimate), " & ", sprintf("%.2f", mean_t), " & ", sig_label, " \\\\")
    )
...
p4 <- ggplot(coef_summary, aes(x = matrix_size_num)) +
    ...
    labs(
        title = "Effect Size by Matrix Size (Averaged Across Blocks)",
        x = "Matrix Size", y = "Coefficient Estimate", color = "Effect"
    ) +
    theme_minimal()
ggsave(here("plots", "h3_effects_by_matrix_size.png"), p4, width = 8, height = 6, dpi = 300)
```
