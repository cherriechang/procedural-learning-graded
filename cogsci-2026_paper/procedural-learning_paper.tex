...
In all blocks across all matrix sizes, this model revealed significant
effects for all predictors (Table \ref{tab:h1h2-results}). This
structure converged without singularity across all 100 models (20 blocks
× 5 conditions) and is therefore used for all subsequent analyses.

\begin{table}[t]
\centering
\caption{Fixed effects from all per-block models (100 total: 20 blocks × 5 matrix sizes)}
\label{tab:h1h2-results}

\begin{tabular*}{\columnwidth}{@{} l @{\extracolsep{\fill}} r r r @{}}
\toprule
Predictor & Mean $\beta$ & Mean \textit{t} & \textit{p} \\
\midrule
Surprisal & 0.025 & 3.16 & $< .001$ \\
Previous Entropy & 0.044 & 2.41 & $< .001$ \\
Surprisal $\times$ Entropy & -0.001 & -0.15 & $< .001$ \\
Repetition & -0.185 & -10.18 & $< .001$ \\
\bottomrule
\end{tabular*}
