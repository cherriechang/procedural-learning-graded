...
contributed to low test-retest reliability of the task
...
mole can appear in. \citet{tab:key-mapping} shows the position-to-key
mapping for each condition.
...
response-stimulus interval (RSI) between trials
(\citet{nissen_attentional_1987}, \citet{janacsek_best_2012},
\citet{howard_implicit_2004}). We find during piloting that allowing
retries, combined with the 120ms RSI, prevents participants from making
compensatory errors, where they unintentionally press the wrong key in
the next trial in an attempt to correct their current trial. We record
response time, keyboard response and correctness for each trial. After
each block, participants are given adaptive feedback based on their
accuracy and speed. At the end of the experiment, participants complete
a brief questionnaire probing their explicit awareness of any patterns
in the mole's appearances (Table).
...
conditions (\citet{tab:key-mapping}). We found during piloting that
