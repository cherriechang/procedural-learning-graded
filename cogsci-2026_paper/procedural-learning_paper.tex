...
\texttt{lmerTest} package in R.P-values were obtained using the
Satterthwaite approximation for degrees of freedom, as implemented in
the \texttt{lmerTest} package (Kuznetsova et al. (2017)).
...
matrix sizes. This comparison favored the model:
\texttt{log\_rt\ \textasciitilde{}\ surprisal\_c\ *\ prev\_entropy\_c\ +\ is\_repetition\ +\ (surprisal\_c\ *\ prev\_entropy\_c\ \textbar{}\textbar{}\ subject\_id)\ +\ (1\ \textbar{}\ position)}

We then fit this model structure each block in each matrix size
condition separately, further pruning the random effets structure until
all models converge. This pruning favored a simpler random effects
structure, resulting in the following final model:
\texttt{log\_rt\ \textasciitilde{}\ surprisal\_c\ *\ prev\_entropy\_c\ +\ is\_repetition\ +\ (prev\_entropy\_c\textbar{}\ subject\_id)\ +\ (1\ \textbar{}\ position)}

In the final block, all predicted effects were significant across all
five matrix size conditions (Table \ref{tab:h1h2-results}). Higher
surprisal transitions elicited slower responses, as did transitions
following higher-entropy positions. The negative interaction indicates
that surprisal effects were stronger following low-entropy (more
predictable) positions, consistent with prediction-based learning.
Repetitions showed faster responses, likely reflecting motor
facilitation. This structure converged without singularity across all
100 models (20 blocks × 5 conditions) and is therefore used for all
subsequent analyses.
...
\footnotesize
\textit{Note.} Mean $\beta$ averaged across 5 matrix size conditions (4×4, 5×5, 6×6, 7×7, 8×8). \\
Sig. = number of conditions with $p$ < .05; *** = all conditions $p$ < .001.
\end{table}
