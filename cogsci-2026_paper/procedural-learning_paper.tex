...
(less predictable) transitions elicit slower and less accurate responses
(surprisal effect). Second, previous positional entropy-----the entropy
of the transition distribution from the previous position-----will
modulate performance independently, with higher-entropy (more uncertain)
previous positions leading to slower and less accurate responses
(entropy effect). We further predict that surprisal effects will be
stronger following low-entropy positions, where predictions are more
confident and prediction errors more costly (surprisal × entropy
interaction). Third, these effects will be attenuated as the number of
positions increases, reflecting greater difficulty in learning more
complex statistical structures (state space size effect).
...
condition (\texttt{surprisal\_c} and \texttt{prev\_entropy\_c},
respectively) to facilitate interpretation of main effects and
interactions in subsequent models. We also create a binary variable
indicating whether the current position is a repetition of the previous
position (i.e.~self-transition). This variable is included as a
covariate in subsequent models to control for potential motor effects
from repeating the same keystroke.

The main dependent variable are response time (RT) and accuracy
...
overall accuracy below 90\%, median RT \textgreater1000ms, or with
...
RTs. Models are fit using the \texttt{lmerTest} package in R.P-values
were obtained using the Satterthwaite approximation for degrees of
freedom, as implemented in the \texttt{lmerTest} package (Kuznetsova et
al. (2017)).
...
Our results provide clear evidence that learners are sensitive to the
graded statistical structure embedded in serial reaction time tasks
across multiple state space sizes. We found robust effects of surprisal
and entropy on reaction times at every matrix size tested (4×4, 5×5,
6×6, 7×7, and 8×8), demonstrating that participants learned fine-grained
transition probabilities rather than simply practicing motor responses
or learning that ``some transitions are hard.'' Critically, we observed
no interaction between surprisal and entropy effects, indicating that
the effect of surprisal persists even when entropy is relatively high.
This independence is particularly impressive because it shows that
learners are not merely learning global difficulty levels, but are
actually tracking which specific transitions are more or less likely
even in high-uncertainty contexts.

\subsection{Scaling Properties and Training
Duration}\label{scaling-properties-and-training-duration}

Importantly, the effects of surprisal and entropy did not rapidly
approach zero as matrix size increased. This suggests that sensitivity
to statistical structure scales beyond the typical 4-position SRTT
paradigm, at least within the range we tested. While there remains
considerable distance between an 8×8 transition matrix and the scale of
statistical structure found in natural language or other real-world
domains, our findings are encouraging given the brevity of training:
participants completed only 20 blocks over approximately 20-40 minutes,
...
patterns across increasing state space complexity.
...
learning. Third, we made methodological decisions---particularly
regarding how to handle random effects structures when fitting separate
models to individual blocks---that may affect the quantitative estimates
of effect sizes, though we believe these are unlikely to alter the
qualitative pattern of results. These modeling choices were necessary to
maintain convergence and interpretability across our per-block analyses,
but future work should explore the robustness of specific effect size
estimates to different modeling approaches.
...
with recent quantitative analyses of language processing
(\textbf{linzen\_how\_2019?}; \textbf{smith\_role\_2013?}). This
...
consistent with theoretical proposals linking the two
(\textbf{ullman\_neurocognition\_2016?};
\textbf{ullman\_declarative\_2004?}).

The preservation of both surprisal and entropy effects across varying
levels of structural complexity is particularly noteworthy. It suggests
that the statistical learning mechanisms engaged in these tasks are not
simply detecting binary patterns or memorizing sequences, but rather
building probabilistic models that capture graded distributional
information. This capacity for nuanced statistical representation in a
procedural learning context provides a bridge between the traditionally
separate literatures on implicit sequence learning and statistical
language learning.
...
larger state spaces (\textbf{sumers\_revisiting\_2024?}). Such designs
would allow us to test whether the scaling properties we observe
continue to hold as we approach state space sizes more representative of
natural language.
...
known about age-related changes in language learning
(\textbf{ullman\_neurocognition\_2016?};
\textbf{hartshorne\_critical\_2018?})---would offer compelling evidence
for or against shared cognitive systems. Such lifespan comparisons could
also inform practical questions about the optimal timing and methods for
language instruction across development.
